pub const DATE_FMT: &str = "%Y-%m-%d";

pub const RETRIES: u32 = 1;
pub const DAYS_AVERAGE: usize = 7;
pub const MAX_FILLED_FOR_DAYS_CUTOFF: f64 = 5.;
pub const RCMND_FILL_DAYS: f64 = 1.;
pub const MARGIN_CUTOFF: f64 = 0.15;
pub const SALES_TAX: f64 = 0.05;
pub const BROKER_FEE: f64 = 0.02;
pub const FREIGHT_COST_ISKM3: f64 = 1500.;
pub const ITEMS_TAKE: usize = 30;
pub const MIN_SRC_VOLUME: f64 = 50.;
pub const MIN_DST_VOLUME: f64 = 0.1;
